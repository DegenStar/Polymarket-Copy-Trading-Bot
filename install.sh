...
print_path_refresh_hint() {
    ...
    if [ ${#PATH_PERSIST_FILES[@]} -gt 0 ]; then
        ...
        echo "export PATH=\"\$HOME/.cargo/bin:\$HOME/.local/bin:\$HOME/bin:\$PATH\""
    fi
}
...
install_rust_toolchain() {
    ...
    if command -v cargo &>/dev/null && command -v rustc &>/dev/null; then
        echo "Rust 工具链已可用：$(command -v cargo)"
        return 0
    fi

    if command -v rustup &>/dev/null; then
        rustup_path="$(command -v rustup)"
        echo "检测到 rustup，正在确保 stable 工具链可用..."
        "$rustup_path" toolchain install stable --profile minimal || true
        "$rustup_path" default stable || true
        ...
        ensure_runtime_path
        if command -v cargo &>/dev/null && command -v rustc &>/dev/null; then
            echo "Rust 工具链已就绪：$(command -v cargo)"
            return 0
        fi
    fi

    installer_path="$(mktemp -t rustup-init.XXXXXX.sh)" || return 1
    if ! download_url_to_stdout 'https://sh.rustup.rs' >"$installer_path"; then
        echo "WARN: 下载 Rust 安装脚本失败：https://sh.rustup.rs" >&2
        rm -f "$installer_path"
        return 1
    fi

    chmod +x "$installer_path"
    /bin/sh "$installer_path" -y --profile minimal --default-toolchain stable
    install_rc=$?
    rm -f "$installer_path"
    ...
    if command -v cargo &>/dev/null && command -v rustc &>/dev/null; then
        echo "Rust 安装完成：$(command -v cargo)"
        return 0
    fi

    echo "WARN: Rust 安装完成后仍未检测到 cargo/rustc。" >&2
    return 1
}

run_step "安装系统依赖" install_dependencies
run_step "安装 Rust 工具链" install_rust_toolchain
ensure_runtime_path
